...
export const fetchDataByGenre = createAsyncThunk("netflix/moviesByGenres",async({genre,type},thunkApi)=>{
    console.log("fetch by genre: ",genre,type);
    const {netflix:{genres},} = thunkApi.getState();
...
const NetflixSlice = createSlice({
    ...
    extraReducers:(buider)=>{
        buider.addCase(getGenres.fulfilled,(state,action)=>{
        ...
        buider.addCase(fetchDataByGenre.fulfilled,(state,action)=>{
            state.movies = action.payload;
        });
        buider.addCase(getUserLikedMovies.fulfilled,(state,action)=>{
            state.movies = action.payload;
        });
        buider.addCase(removeFromLikedMovies.fulfilled,(state,action)=>{
            state.movies = action.payload;
        });
    },
});
